...
| 種類 | 格式 | 備註 |
|------|------|------|
| 二維碼（QR） | 結構特徵：空白+任一字母(標記)+15碼數字+12碼料號的長字串 | 可擷取數量（`expectedQty`）；料號為標記後第 16~27 碼（`/\s[A-Za-z]\d{15}(.{12})/` 的 `m[1]`），與前綴（如 `1AC`／`2AG`／`2AB`／`2ZF`）、type 欄位（`J004`／`S912`）、標記字母（`E`／`F`）無關，故不需為新前綴/客戶維護白名單 |
